...
router.post('/', async (req, res) => {
  try {
    ...
    let { farmerName, aadharNumber, pesticides } = req.body;

    // Handle nested data structure if pesticides is not at top level
    if (!pesticides && req.body.data && req.body.data.pesticides) {
      // Map frontend structure to backend model structure
      pesticides = req.body.data.pesticides.map(p => ({
        pesticideName: p.name,
        pesticideType: 'Organic', // Default type since frontend doesn't send it
        quantity: p.quantity
      }));
    } else if (pesticides) {
      // Ensure existing pesticides have correct fields if they come from another source
      pesticides = pesticides.map(p => ({
        pesticideName: p.pesticideName || p.name,
        pesticideType: p.pesticideType || 'Organic',
        quantity: p.quantity
      }));
    } else {
      pesticides = [];
